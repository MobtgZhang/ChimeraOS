/// 8x16 bitmap font for GUI text rendering.
/// Contains bitmaps for ASCII 32 (space) through 126 (~).
/// Each glyph is 8 pixels wide and 16 pixels tall, stored as 16 bytes
/// where each byte represents one row (MSB = leftmost pixel).
...
pub fn getGlyph(ch: u8) ?*const [16]u8 {
    if (ch < FIRST_CHAR or ch > LAST_CHAR) return null;
    return &font_data[ch - FIRST_CHAR];
}

// Standard VGA 8x16 bitmap font — public domain
const font_data = [_][16]u8{
    // 32 ' '
